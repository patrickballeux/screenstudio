...
public class DesktopViewer implements Runnable {
    ...
    @Override
    public void run() {
        if (!mDevice.getId().equals("WEBCAM")) {
            ...
            try {
                ...
                if (mDevice.getId().endsWith(".screen")) {
                    ...
                    command = bin + " -nostats -loglevel 0 -f " + displayFormat + " -video_size " + buffer.getWidth() + "x" + buffer.getHeight() + " -i " + mDevice.getId()+"+" + (int) mDevice.getSize().getX() + "," + (int) mDevice.getSize().getY() + "  -r " + mDevice.getFps() + "  -f rawvideo -pix_fmt bgr24 -";
                }
